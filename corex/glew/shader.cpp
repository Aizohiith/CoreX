...
void Shader::Set_Attribute(GLuint pi_ID, float pf_Value) const
{
	glUniform1f(pi_ID, pf_Value);
}
void Shader::Set_Attribute(GLuint pi_ID, const glm::vec2& pv_Value) const
{
	glUniform2f(pi_ID, pv_Value.x, pv_Value.y);
}
void Shader::Set_Attribute(GLuint pi_ID, const glm::vec3& pv_Value) const
{
	glUniform3f(pi_ID, pv_Value.x, pv_Value.y, pv_Value.z);
}
void Shader::Set_Attribute(GLuint pi_ID, const glm::vec4& pv_Value) const
{
	glUniform4f(pi_ID, pv_Value.x, pv_Value.y, pv_Value.z, pv_Value.w);
}
void Shader::Set_Attribute(GLuint pi_ID, const glm::mat4& pm_Value) const
{
	glUniformMatrix4fv(pi_ID, 1, GL_FALSE, glm::value_ptr(pm_Value));
}

void Shader::Set_Attribute(GLuint pi_ID, const GLuint pi_TextreUnit) const
{
